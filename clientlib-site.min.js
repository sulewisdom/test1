$(document).ready(function(){
  ...
  $('#toggle').click(function () {
    ...
    $(this).parents('.mobile-nav-container').find('.default-top').toggleClass('open');
      var getlogo = $('.mobile-nav-logo img').attr('data-logo');
      var getlogoWhite = $('.mobile-nav-logo img').attr('data-logo-white');
      if($(this).hasClass('active')){
        $('.mobile-nav-logo img').attr('src',getlogoWhite);
      }else{
        $('.mobile-nav-logo img').attr('src',getlogo);
      }
  });
  
  $('body').on('keypress','#toggle', function(e){
    if (e.which == 13) {
      $(this).trigger('click');
    }
  });
  ...
  // if ($('.nav-item').children('.dropdown-submenu').css('display') === 'block') {
  ...
  $(".overlay-menu .mobilesubmenu").on('show.bs.collapse', function () {
    $(this).parents('li').find('.open-submenu').css('display', 'none');
    $(this).parents('li').find('.close-submenu').css('display', 'inline-block');
    $(this).parents('li').find('.close-submenu svg').css('transform', 'rotate(45deg)');
  });
  $(".overlay-menu .mobilesubmenu").on('hide.bs.collapse', function () {
    $(this).parents('li').find('.open-submenu').css('display', 'inline-block');
    $(this).parents('li').find('.close-submenu').css('display', 'none');
    $(this).parents('li').find('.close-submenu svg').css('transform', 'rotate(0deg)');
  })
  ...
  if($(window).width() >768){
    if($('#scrollId').length){
      var scrollId = document.querySelector('#scrollId');
      var myScrollFunc = function () {
        var y = window.scrollY;
        if (y >= 2500) {
      
          scrollId.className = "scrolltop scrollshow";
        } else {
      
          scrollId.className = "scrolltop scrollhide";
        }
      };
  ...
  function headerDropShadow(){
  var myheaderFunc = function () {
      var y = window.scrollY;
      if($(window).width() > 1024){
        if (y >= 50) {
            document.querySelector('header').className = "main-header navbar-fixed-top header-dropshadow";
        } else {
            document.querySelector('header').className = "main-header navbar-fixed-top";
        }
      }else{
        if (y >= 50) {
            document.querySelector('.mobile-nav-container').className = "mobile-nav-container header-dropshadow";
        } else {
            document.querySelector('.mobile-nav-container').className = "mobile-nav-container";
        }
      }
  };
  
  window.addEventListener("scroll", myheaderFunc);
  }
  ...
  $('.btn-search').click(function() {
      ...
      var searchInpTxt = $(this).parents('form').find('.searchtxt');
      var searchVal = searchInpTxt.val();
      if(searchVal != ""){
          searchVal = escapeHTML(searchVal);
          var deafultLink = "/search/";
          if($(searchInpTxt).val().length >= 2 && searchVal.length >= 2){
              var siteUrl = escapeHTML(deafultLink);
              window.location.href = siteUrl+'?s='+escape(encodeURIComponent(escapeHTML($(searchInpTxt).val())));
              return false;
          }
      }
   });
  function escapeHTML(str){
      ...
      if (e.which == 13) {
          $(this).parents('form').find('.btn-search').trigger('click');
      }
  });
  ...
  $(this).button('loading');
      var blogInpTxt = $(this).parents('form').find('input[type=text]');
      var blogSearchVal = blogInpTxt.val();
      if(blogSearchVal != ""){
          blogSearchVal = escapeHTML(blogSearchVal);
          var deafultLink = "/search/";
          if($(blogInpTxt).val().length >= 2 && blogSearchVal.length >= 2){
              var siteUrl = escapeHTML(deafultLink);
              window.location.href = siteUrl+'?type=blog&s='+escape(encodeURIComponent(escapeHTML($(blogInpTxt).val())));
              return false;
          }
      }
   });
  $('.blog-input-text').keyup(function(){
      var searchVal = $(this).val().match(/[^ ]/g);
      console.log(searchVal);
      if($(this).val().length >= 2 && searchVal.length >= 2){
          $('.blog-search-button').attr('disabled', false);
      }else{
          $('.blog-search-button').attr('disabled', true);
      }
  });
  // Accessibility
  // ==========
  ...
  $(document).ready(function () {
      $('.skipNavLink').focus(function() {
          ...
          $(this).parents('body').addClass('accessibility-active');
      });
  
      $('.skipNavLink').blur(function() {
            $(this).parents('body').removeClass('accessibility-active');
      });
  });
  $(document).ready(function () {
      if($('header').parents('.xf-content-height').length > 0) {
          $('header').parents('.xf-content-height').addClass("stickyHeader");
      }
  
      $('[data-target="#globalModal"]').click(function(){
            $('.xf-content-height.stickyHeader').removeClass('stickyHeader');
      });
      $('#globalModal .close').click(function(){
          $(this).parents('.xf-content-height').addClass('stickyHeader');
      });
  });
  if ($('.vidyard').length>0){
  ...
  var loadAccordion = 1;
  var headerHeight= 0;
  var lengthCard, cardindex = 0;
  var autoInterval, animateInterval = "";
  var funcCall = 0;
  ...
  $(document).ready(function () {
      ...
      if ($(window).width() > 1024) {
            ...
            headerHeight = $('.mobile-nav-container').outerHeight();
       }
  
       $('.tabs-progressbar .custom-accordion .card-header a[data-toggle="collapse"]').click(function (event) {
            var _this = $(this);
            var getval = $(this).parents('.card').attr('data-perc');
            if (_this.parents('.tabs-progressbar').find('.accord-progressbar').length) {
                 _this.parents('.tabs-progressbar').find('.accord-progressbar').css('width', getval + "%");
            }
  
            if ($(this).parents('.card').attr('data-tabimg') != undefined) {
                 if ($(this).parents('.card-header').next('.collapse').hasClass('show')) {
                      event.stopPropagation();
                 }
                 // Dynamic Image Path Append
                 var getImg = $(this).parents('.card').attr('data-tabimg');
                 $(this).parents('.tabs-progressbar').find('.prog-img').attr('src', getImg);
  
                 // Dynamic Title and Alt Image
                 var getAltText = $(this).parents('.card').attr('data-alt');
                 if (getAltText != undefined) {
                      $(this).parents('.tabs-progressbar').find('.prog-img').attr('title', getAltText);
                      $(this).parents('.tabs-progressbar').find('.prog-img').attr('alt', getAltText);
                 }
                 // Dynamic Title and Caption Image
                 var imageCaption = $(this).parents('.card').attr('data-caption');
                 var getCaptionHeight = 0;
                 if (imageCaption != undefined) {
                      $(this).parents('.tabs-progressbar').find('.image-caption').show();
                      $(this).parents('.tabs-progressbar').find('.image-caption p').text(imageCaption);
                      getCaptionHeight = - parseInt($(this).parents('.tabs-progressbar').find('.image-caption').outerHeight() / 2);
                      $(this).parents('.tabs-progressbar').find('.image-caption').css('margin-top', getCaptionHeight);
  
                 } else {
                      $(this).parents('.tabs-progressbar').find('.image-caption').hide();
                 }
  
  
                 // Tab Indexing Focus
                 $(this).attr('tabindex', '-1').parents('.card').siblings().find('a[data-toggle="collapse"]').removeAttr('tabindex');
                 if ($(this).is(":focus")) {
                      $(this).css({ 'border-width': '0px' }).parents('.card').siblings().find('a[data-toggle="collapse"]').removeAttr('style');
                 }
  
  
                 // Auto Scroll
                 if (_this.parents('.accord-autoScroll').length) {
                      _this.parents('.card').addClass('active').siblings().removeClass('active');
                      cardindex = _this.parents('.card').index();
  
                      _this.parents('.card').find('.progressbar-animate span').removeAttr('style');
                       _this.parents('.card').find('.progressbar-animate span').css({ 'animation-play-state': 'running', 'animation-duration': '8000ms', 'animation-timing-function': 'linear' }).parents('.card').siblings().find('.progressbar-animate span').css({ 'animation-play-state': 'paused', 'animation-name': 'none' });
                 }
                 ...
                 if ($(this).parents('.card-header').next('.collapse').hasClass('show')) {
                      $(this).parents('.tabs-progressbar').find('.accord-progressbar').css('width', "0%");
                 }
                 if($(this).hasClass('collapsed') && loadAccordion > 1){
                     setTimeout(function(){
                          $('html, body').animate({
                               'scrollTop': _this.parents('.card-header').offset().top - headerHeight - 20
                          }, 500);
                     },500)
                }
                loadAccordion ++;
       ...
       $('.tabs-progressbar').each(function () {
            var _this = $(this);
            var cardlen = 100 / $(this).find('.custom-accordion >.card').length;
            for (i = 0; i <= $(this).find('.custom-accordion >.card').length; i++) {
                 var count = i + 1;
                 var valAssign = cardlen * count
                 $(this).find('.custom-accordion >.card').eq(i).attr('data-perc', valAssign);
            }
            setTimeout(function () {
                 if (!_this.find('.noprogressbar').length) {
                      _this.find('.custom-accordion .card-header a[data-toggle="collapse"]').eq(0).click();
            }
            }, 500)
  
       });
      if($('.videoplay-icon').length > 0) {
  
      $('.videoplay-icon').each(function(){
          ...
          $('video').click(function () {
               if ($(this).get(0).paused) {
                    $(this).parents('.videoplay-icon').addClass('active');
               }else{
                    $(this).parents('.videoplay-icon').removeClass('active');
               }
          });
          $(document).on('click', '.play-icon', function () {
               $(this).parents('.videoplay-icon').find('video').trigger('click');
               $(this).parents('.videoplay-icon').find('video')[0].play();
          })
       ...
       $('.custom-styling .form-label-group .form-control').each(function () {
            if ($(this).attr('placeholder') != undefined) {
                 var getlength = $(this).attr('placeholder').length;
                 var getLabelText = "";
                 if (getlength >= 35) {
                      getLabelText = $(this).next('label');
                      $(this).parents('.form-group').find('>div').removeClass('form-label-group');
                      getLabelText.insertBefore(this);
                      getLabelText.addClass('p12 text-gray3');
                      $(this).attr('placeholder', '');
                 }
            }
       })
  
       // Form File Type Floating
  
       $('input[type=file]').change(function (e) {
            var getVal = e.target.files[0].name;
            $(this).parents('.custom-file').find('.custom-file-label').text(getVal);
            if (getVal != "") {
                 $(this).parents('.custom-file').find('label').addClass('active');
            } else {
                 $(this).parents('.custom-file').find('label').removeClass('active');
            }
       })
  
       // Search Popup
  
       $('.search-container .searchbtn').click(function () {
            $(this).parents('.search-container').toggleClass('openSearch')
            $(this).find('i').toggleClass('fa-close');
  
            if ($(this).parents('.search-container').hasClass('openSearch')) {
                 $(this).parents('.search-container').find('input[type=search]').attr('tabindex', '0');
                 $(this).parents('.search-container').find('.btn-search').attr('tabindex', '0');
                 $(this).parents('.search-container').find('input[type=search]').focus();
            } else {
                 $(this).parents('.search-container').find('input[type=search]').attr('tabindex', '-1');
                 $(this).parents('.search-container').find('.btn-search').attr('tabindex', '-1');
            }
       });
  
       $('.search-container .btn-search, .blog-global-search .blog-search-button').click(function () {
  
            if($(this).parents('form').find('input').val() !=""){
                 var getSpinner = $(this).attr('data-loading-text');
                 $(this).html(getSpinner)
            }
       });
       $('.search-container .search-input input').keyup(function (e) {
           if (e.which == 27){
              $('.search-container .searchbtn').trigger('click');
              $('.search-container .searchbtn').focus();
           }
       });
      $(document).click(function(e) {
       ...
       $(document).on('click', '.clickable_Card', function(){
        ...
        $(document).on('keypress', '.clickable_Card', function(e){
               if (e.which === 13) {
                  var getlink = $(this).attr('data-href');
                  var getTarget = $(this).attr('data-target');
                  if(getlink != undefined){
         ...
         $(document).on('click', '.clickable_Card a', function (e) {
       ...
       // Image Frame arrow div
  
        $('.frame-img.frame-arrow').each(function(){
             $(this).append('<span class="triangular"></span>');
       });
  
  
       // Slidebar Dropdown
  
       $(document).on('click', '.slidebarDropdown', function (e) {
       ...
       $(document).on('click', '.sidebar >ul >li a', function (e) {
            ...
            if ($(window).width() < 767) {
                 ...
                 $(this).parents('.sidebar').find('.slidebarDropdown p .drop-text').text(getval);
                 $(this).parents('.sidebar').find('>ul').slideUp();
                 $(this).parents('.sidebar').find('.slidebarDropdown').removeClass('open');
            }
       });
       $('.sidebar >ul >li.active a').trigger('click');
  
       $(document).on('click', '.filter-Section .filter-dropdown', function (e) {
            $(this).toggleClass('open');
            $(this).next('ul').slideToggle();
       });
       $(document).on('click', '.filter-Section >ul >li a', function (e) {
            $(this).parents('li').addClass('active').siblings().removeClass('active');
            $(this).parents('.filter-Section').find('>ul').slideUp();
            $(this).parents('.filter-Section').find('.filter-dropdown').removeClass('open');
       });
  
       // Modal Popup Enter
       $(document).on('keypress', '[data-toggle=modal]', function (e) {
            if (e.which === 13) {
                 $(this).trigger('click');
            }
       });
        // This for replace class container to container-fluid
            // it will not be present in prototype
           $('.customrte .container').each(function(){
           $(this).addClass('container-fluid').removeClass('container');
  
       });
       //This is for dynamic data manipulation for resource or blog details pages
       // it will not be present in prototype
      $('.detail-body p').each(function(){
  ...
  function customSelectDropdown(){
       ...
       $('.custom-styling select').each(function () {
       ...
       $('.custom-styling select').change(function () {
            if (!$(this).find('option:first-child:selected').length) {
                 $(this).parents('.selectWrap').addClass('active');
            } else {
                 $(this).parents('.selectWrap').removeClass('active')
  ...
  function addArrowNavigation() {
       $('.main-header .navbar-nav .dropdown').each(function () {
            var _this = $(this);
            if (!_this.find('.dropdown-arrow').length != "") {
                 _this.find('.dropdown-submenu').append('<span class="dropdown-arrow"></span>');
            }
            var offsetelement = _this.find('.nav-link').position().left;
            var getWidthelement = _this.find('.nav-link').outerWidth(true) / 2;
            var arrowposition = offsetelement + getWidthelement;
            var dropWidth = 0;
  
  
            if (_this.parents('.nav-main').length) {
                 dropWidth = $('.container-fluid').width();
            }
            else {
                 dropWidth = 300;
            }
            _this.find('.dropdown-submenu').css({ '--siteMenuArrowOffset': arrowposition + "px", '--siteMenuWidth': dropWidth + "px" });
  
            setTimeout(function(){
                 var dropHeight = _this.find('.dropdown-submenu .dropdowncontent').outerHeight() + 20;
                 _this.find('.dropdown-submenu').css({ '--siteMenuHeight': dropHeight + "px" });
  ...
  function headernavigation() {
       ...
       $('.main-header nav.navbar').on('mouseenter focus', '.nav-item.dropdown > .nav-link', function (e) {
            var el = $(this);
            ...
            el.next('.dropdown-menu').find('.btn, a').attr('tabindex', '0');
            if (el.parents('.nav-main').length) {
                 $('body').addClass('dropdown-open');
            }
                 el.parents('.dropdown').removeClass('menu-leftanimate menu-rightanimate');
            el.parents('.dropdown').prevAll('.dropdown').addClass('menu-leftanimate');
            el.parents('.dropdown').nextAll('.dropdown').addClass('menu-rightanimate');
            }, this), 120));
  
       }).on('mouseleave blur', '.nav-item.dropdown > .nav-link', function (e) {
            clearTimeout($.data(this, "timer"));
            var el = $(this);
            el.toggleClass('has-focus');
            $('.dropdown').removeClass('menu-leftanimate menu-rightanimate');
            // Only hide sub-menu after a short delay, so links get a chance to catch focus from tabbing
            setTimeout(function () {
                 if (el.siblings('.dropdown-menu').attr('data-has-focus') !== 'true') {
                      el.attr('aria-expanded', 'false');
                      el.parents('.dropdown').removeClass('active');
                      el.next('.dropdown-menu').find('.btn, a').attr('tabindex', '-1');
  
                 }
                 if (el.parents('.nav-main').find('.dropdown.active').length === 0) {
            ...
            setTimeout(function () {
                 ...
                 if (el.find(':focus').length === 0) {
                      ...
                      if (el.siblings('.nav-link.has-focus').length === 0) {
                           el.parents('.dropdown').find('> .nav-link').attr('aria-expanded', 'false');
                           el.parents('.dropdown').removeClass('active');
                           el.find('.btn, a').attr('tabindex', '-1');
                      }
                 }
                 if (el.parents('.nav-main').find('.dropdown.active').length === 0) {
                      $('body').removeClass('dropdown-open');
  ...
  function herobanner(){
       // Banner Alignment Arrow
  
       if ($('.banner-container').length) {
            if ($(window).width() > 1025) {
                 var getwidth = $('header .nav-main').find('.navbar-nav').outerWidth() - 15;
                 $('.banner-shortheader-content').find('.right-box').css('width', getwidth);
  
            }else{
                 $('.banner-shortheader-content').find('.right-box').removeAttr('style')
            }
       }
  }
  ...
  function globalteam() {
       ...
       $(document).on('click', '.global-team-section .card', function (e) {
            var gethtml = $(this).next('.hidden-box').html();
            $('#global-team-modal').find('.modal-body').html(gethtml);
            $('.modal-backdrop').addClass('global-overlay');
  ...
  function timeline() {
       if ($('.site-timeline').length) {
            var $timeline_block = $('.animatediv');
  
            //hide timeline blocks which are outside the viewport
            $timeline_block.each(function () {
                 if ($(this).offset().top > $(window).scrollTop() + $(window).height() * 0.75) {
                      $(this).find('.site-timeline-img, .site-timeline-content').addClass('is-hidden');
                 }
            });
            ...
            $(window).on('scroll', function () {
                 $timeline_block.each(function () {
                      if ($(this).offset().top <= $(window).scrollTop() + $(window).height() * 0.75 && $(this).find('.site-timeline-img').hasClass('is-hidden')) {
                           $(this).find('.site-timeline-img, .site-timeline-content').removeClass('is-hidden').addClass('bounce-in');
                      }
                 });
            });
  
            for (i = 0; i < $('.site-timeline .site-timeline-block').length; i++) {
                 $('.site-timeline-block:odd').addClass('align-right')
            }
  
            var lastEleHeight = $('.site-timeline .site-timeline-block:last-child').outerHeight() / 2;
            $('.site-timeline .timeline-border').css('height', 'calc(100% - ' + lastEleHeight + 'px)')
       }
  }
  
  function autoScrollAccordion(){
       $('.accord-autoScroll').each(function(){
            lengthCard = $(this).find('.card').length;
            $(this).attr('data-cardlength',lengthCard);
                 timerAutoAccord();
        });
  
       $('.tabs-progressbar').on('mouseenter focus', '.accord-autoScroll', function (e) {
            var _this = $(this);
            _this.addClass('hoverstate');
       clearInterval(autoInterval);
  
       }).on('mouseleave blur', '.accord-autoScroll', function (e) {
            var _this = $(this);
            _this.removeClass('hoverstate');
            _this.find('.card').find('.progressbar-animate span').removeAttr('style');
            _this.find('.card.active').find('.progressbar-animate span').css({ 'animation-play-state': 'running', 'animation-duration': '8000ms', 'animation-timing-function': 'linear' }).parents('.card').siblings().find('.progressbar-animate span').css({ 'animation-play-state': 'paused', 'animation-name': 'none' });
                 clearInterval(autoInterval);
       timerAutoAccord();
  
       })
   }
  function timerAutoAccord(){
            autoInterval = setInterval(function () {
            if (cardindex < parseInt($('.accord-autoScroll').attr('data-cardlength')) ) {
                 $('.accord-autoScroll').find('.card').eq(cardindex).find('.card-header a[data-toggle="collapse"]').trigger('click');
            } else {
                 $('.accord-autoScroll').find('.card').eq(0).find('.card-header a[data-toggle="collapse"]').trigger('click');
  ...
  window.onresize = function() {
       ...
       if ($('.sidebar').length) {
            if ($(window).width() > 767) {
                 ...
                 $('.slidebarDropdown').removeClass('open');
            }
   }
  
  }
  $(window).on('resize scroll', function() {
  if($('.accord-autoScroll').length){
       if ($('.accord-autoScroll').isInViewport()) {
            funcCall ++;
            if(funcCall == 1){
                 clearInterval(autoInterval);
                 $('.accord-autoScroll').removeClass('hoverstate');
                 $('.accord-autoScroll').find('.card.active').find('.progressbar-animate span').css({ 'animation-play-state': 'running', 'animation-duration': '8000ms', 'animation-timing-function': 'linear' }).siblings().find('.progressbar-animate span').css({ 'animation-play-state': 'paused', 'animation-name': 'none' });
                timerAutoAccord();
            }
  
       } else {
            funcCall=0;
            $('.accord-autoScroll').addClass('hoverstate');
            clearInterval(autoInterval);
       }
       }
   });
